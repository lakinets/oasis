...
<table class="table">
    ...
    <tbody>
        <?php if (!empty($richPlayers)): ?>
            <?php foreach ($richPlayers as $i => $row): ?>
                <tr class="<?= $i % 2 == 0 ? 'odd' : 'even' ?>">
                    ...
                    <td><?= number_format($row['adena_count'], 0, '', '.') ?></td>
                    <td>
                        <?= $row['online'] 
        ...
        <?php else: ?>
            <tr>
                <td colspan="7"><?= Yii::t('main', 'Данных нет') ?></td>
            </tr>
        <?php endif; ?>
    </tbody>
</table>
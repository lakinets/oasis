...
class PainTeamIt
{
    ...
    public function characters(): Query
    {
        return (new Query())
            ...
            ->leftJoin('character_variables cv', 'cv.charId = characters.obj_Id AND cv.name = "jail"');
    }
    ...
    public function multiInsertItem(array $items): bool
    {
        ...
        return $this->db->createCommand()
            ->batchInsert('character_donate', ['obj_Id', 'char_name', 'item_id', 'count', 'issued'], $rows)
            ->execute() > 0;
    }
    ...
    public function getCastles(): array
    {
        return (new Query())
            ->select([
                'castle.id', 'castle.name', 'castle.tax_percent AS taxPercent', 'castle.siege_date AS siegeDate',
                'clan_data.clan_id', 'clan_data.clan_name', 'clan_data.clan_level', 'clan_data.reputation_score',
                'clan_data.hasCastle', 'clan_data.ally_id', 'clan_data.leader_id',
                'clan_data.crest AS clan_crest', 'clan_data.largecrest AS clan_crest_large',

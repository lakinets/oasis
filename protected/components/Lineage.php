...
class Lineage
{
    ...
    private function loadQuery()
    {
        ...
        {
            throw new CException(Yii::t('main', 'Файл с запросами не найден :file', array(':file' => Yii::getPathOfAlias('app.l2j.' . $className))));
        }
    ...
    public function __call($method, $params)
    {
        ...
        {
            return call_user_func_array(array($this->_query, $method), $params);
        }
    ...
    {
        if($this->config('password_type') == 'wirlpool')
        {
    ...
    public static function getGradeName($crystal_type)
    {
        ...
        switch($crystal_type)
        {
            ...
            case 4: $symbol = 'A'; break;
            case 5: $symbol = 'S'; break;
            case 6: $symbol = 'S80'; break;
            case 8: $symbol = 'R'; break;
            case 9: $symbol = 'R95'; break;
            case 10: $symbol = 'R99'; break;
        }
    ...
    public static function getItemLocName($loc)
    {
        ...
        {
            case 'INVENTORY':
                return Yii::t('main', 'Инвентарь');
            case 'PAPERDOLL':
                return Yii::t('main', 'Одето');
            case 'WAREHOUSE':
                return Yii::t('main', 'На складе');
            default:
                return $loc;
        }
    }
    ...
    public static function getItemIcon($icon, $title = '')
    {
        ...
        {
            return CHtml::image($url . $icon . '.jpg', $title, array('title' => $title));
        }

        return CHtml::image($url . 'no-image.jpg', $title, array('title' => $title));
    }
    ...
    {
        return $sex == 1 ? Yii::t('main', 'женщина') : Yii::t('main', 'мужчина');
    }
    ...
    public static function statsName($type)
    {
        ...
        {
            case 'total':
                return Yii::t('main', 'Общая');
            case 'pvp':
                return Yii::t('main', 'ПВП');
            case 'pk':
                return Yii::t('main', 'ПК');
            case 'clans':
                return Yii::t('main', 'Кланы');
            case 'castles':
                return Yii::t('main', 'Замки');
            case 'online':
                return Yii::t('main', 'В игре');
            case 'top':
                return Yii::t('main', 'ТОП');
            case 'rich':
                return Yii::t('main', 'Богачи');
            case 'items':
                return Yii::t('main', 'Предметы');
            default:
                return 'n/a';
        }
    }
    ...
    public static function getCastleIcon($castleId)
    {
        ...
        return CHtml::image(app()->createUrl('images/castles') . $castleId . '.jpg', $castleName, array('title' => $castleName));
    }
}

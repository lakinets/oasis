...
class ReferalsProfit extends ActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('referer_id, referal_id, profit, sum, percent, created_at', 'required'),
			array('profit, sum, percent', 'numerical'),
			array('referer_id, referal_id', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, referer_id, referal_id, profit, sum, percent, created_at', 'safe', 'on'=>'search'),
		);
	}
	...
	{
		return array(
            'user' => array(self::HAS_ONE, 'Users', array('user_id')),
		);
	}
